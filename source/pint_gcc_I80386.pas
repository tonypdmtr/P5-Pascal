...
procedure load;
   ...
   procedure assemble; (*translate symbolic code into machine code and store*)
      ...
      begin
        ...
        if ep^.r <> nil then gentre(ep^.r);
        case op of { expression operator }
          0{lodi}, 105{loda}, 4{lda}: begin r := ord(rgebp); 
            if ep^.p > 0 then begin r := ep^.rt; getdsp(p, r) end;
            wrtins(prr, '        mov     #1(%r1),%r2    ', q, 0, r, ep^.rr);

          end;
          193{lodx}, 108{lodb}, 109{lodc}: begin r := rgebp; 
            if ep^.p > 0 then begin r := ep^.rt; getdsp(p, r) end;
          ...
          107{lods}: begin if ep^.p > 0 then getdsp(p, ord(rgeax));
            ...
            wrtins(prr, '        call     _passup_pshset  ', q, 0, r, 0);
            wrtins(prr, '        mov     %esp,%eax        ', 0, 0, ep^.rt, 0); 
          end;
